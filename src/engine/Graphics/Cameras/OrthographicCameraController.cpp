...
void OrthographicCameraController::OnUpdate(Timestep ts)
{
    if (InputManager::GetInstance().IsKeyPressed(KEYBOARD_BTN_A)){
        m_CameraPosition.x -= cos(glm::radians(m_CameraRotation)) * m_CameraTranslatedSpeed * ts;
        m_CameraPosition.y -= sin(glm::radians(m_CameraRotation)) * m_CameraTranslatedSpeed * ts;
    ...
    if (InputManager::GetInstance().IsKeyPressed(KEYBOARD_BTN_W)){
        m_CameraPosition.x += -sin(glm::radians(m_CameraRotation)) * m_CameraTranslatedSpeed * ts;
        m_CameraPosition.y += cos(glm::radians(m_CameraRotation)) * m_CameraTranslatedSpeed * ts;
